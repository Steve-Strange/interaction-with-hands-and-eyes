...
public class SightConeSelectOnly : MonoBehaviour
{
    ...
    IEnumerator UpdateObjectWeights()
    {
        while (true)
        {
            ...
            if (!HandPoseManager.GetComponent<HandPoseManagerSelectOnly>().SecondSelectionState
                && EyeTrackingManager.GetComponent<EyeTrackingManagerSelectOnly>().isEyesOpen)
            {
                // 获取视线中心位置
                foreach (GameObject obj in selectedObjects)
                {
                    // 计算物体到视线中心的距离
                    float distance = DistanceToLineOfSight(obj.transform.position, transform.position,
                        EyeTrackingManager.GetComponent<EyeTrackingManagerSelectOnly>().combineEyeGazeVectorInWorldSpace);

                    objectWeights[obj] += Mathf.Exp(-distance * distance * selectedObjects.Count / 2) * 2;
    ...
    void OnTriggerEnter(Collider other)
    {
...

        {
            ...
            if (hitObj == EyeTrackingManager.GetComponent<EyeTrackingManagerSelectOnly>().eyeSelectedObject) return;
